namespace OmegaRace
{
    public class GameScenePlay : IGameScene
    {
        ...
        void IGameScene.Update()
        {
            ...
            {
                dataMsg = new MovementMessage(DataMessage.PlayerID.Player2,
            ...
            {
                dataMsg = new FireMessage(DataMessage.PlayerID.Player2);
                MsgQueueMgr.AddToOutputQueue(dataMsg);
                NetMgr.SendMessage(dataMsg);
            }
            if (InputManager.GetButtonDown(INPUTBUTTON.P2_LAYMINE))
            {
                dataMsg = new MineMessage(DataMessage.PlayerID.Player2);
                MsgQueueMgr.AddToOutputQueue(dataMsg);
                NetMgr.SendMessage(dataMsg);
            }
