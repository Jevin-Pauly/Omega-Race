...
namespace OmegaRace
{
    public class GameScenePlay : IGameScene
    {
        ...
        void BroadcastPrediction(ref int ch)
        {
            Ship p1 = PlayerMgr.P1Data.ship;
            Ship p2 = PlayerMgr.P2Data.ship;

            // Only send update if error exceeds prediction threshold (1.0 pixel)
            if ((p1.prediction.GetPredictedPosition() - p1.GetPixelPosition()).Length() > 1.0f)
            {
                Vec2 misslePos = p1.GetPixelPosition();
                Vec2 missleVel = p1.GetPixelVelocity();
                float serverTime = TimeManager.GetCurrentTime();
                p1.prediction.UpdateFromServer(misslePos, missleVel, serverTime);
                MsgQueueMgr.AddToOutputQueue(PredictionMessage.Create(p1.getID(), misslePos, missleVel, serverTime, ch++));
            }
            // Only send update if error exceeds prediction threshold (1.0 pixel)
            if ((p2.prediction.GetPredictedPosition() - p2.GetPixelPosition()).Length() > 1.0f)
            {
                Vec2 misslePos = p2.GetPixelPosition();
                Vec2 missleVel = p2.GetPixelVelocity();
                float serverTime = TimeManager.GetCurrentTime();
                p2.prediction.UpdateFromServer(misslePos, missleVel, serverTime);
                MsgQueueMgr.AddToOutputQueue(PredictionMessage.Create(p2.getID(), misslePos, missleVel, serverTime, ch++));
            }
            foreach (Missile m in PlayerMgr.P1Data.missileList)
            {
                Vec2 misslePos = m.GetPixelPosition();
                Vec2 missleVel = m.GetPixelVelocity();
                // Only send update if error exceeds prediction threshold (1.0 pixel)
                if ((m.prediction.GetPredictedPosition() - misslePos).Length() > 1.0f)
                {
                    float serverTime = TimeManager.GetCurrentTime();
                    m.prediction.UpdateFromServer(misslePos, missleVel, serverTime);
                    MsgQueueMgr.AddToOutputQueue(PredictionMessage.Create(m.getID(), misslePos, missleVel, serverTime, ch++));
                }
            }
